...
resource "aws_cloudfront_distribution" "roboshop" {
  ...
  default_cache_behavior {
    ...
    target_origin_id = "${var.project_name}-${var.environment}.${var.domain_name}" # comes from above origin,  origin_id  

    viewer_protocol_policy = "https-only"
    cache_policy_id        = local.cachingDisabled  # cacheing vadhu 
  }

  # Cache behavior with precedence 0  ( means oders wise cacheing)
  ordered_cache_behavior {
    ...
    viewer_protocol_policy = "https-only"
    cache_policy_id        = local.cachingOptimised  # cacheing yes 
  }
